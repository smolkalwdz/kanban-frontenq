...
const Board: React.FC<BoardProps> = ({ onOpenAdmin }) => {
  ...
  // Загрузка зон и бронирований с backend
  useEffect(() => {
  ...
  useEffect(() => {
    setForm((prev) => {
      ...
      return {
        ...prev,
        // Обновляем tableId только если текущая зона недоступна
        tableId: currentTableExists ? prev.tableId : (currentTables[0]?.id || 1)
      };
    });
  }, [currentBranch, currentTables]);
  ...
  const handleDrop = (tableId: number) => {
    if (draggedBooking) {
      setBookings((prev) =>
